...
export class VirtualFS {
  ...
  resolvePath(inputPath) {
    ...
    // Home shortcut
    if (inputPath === "~") return "/home/player";
    if (inputPath.startsWith("~/")) {
      return this._normalize("/home/player/" + inputPath.slice(2));
    }
  ...
  ls(args = []) {
    ...
    if (node.type === "file") {
      if (showLong) {
        ...
        return { success: true, output: `-${perms} 1 player player 128 Jan 15 10:00 ${targetPath.split("/").pop()}` };
      }
    ...
    if (showLong) {
      const lines = [];
      if (showHidden) {
        lines.push(`drwxr-xr-x 2 player player 4096 Jan 15 10:00 .`);
        lines.push(`drwxr-xr-x 3 player player 4096 Jan 15 10:00 ..`);
      }
      for (const child of allChildren.filter(c => c !== "." && c !== "..")) {
        ...
        lines.push(`${prefix}${perms} 1 player player ${size.padStart(4)} Jan 15 10:00 ${display}`);
      }
  ...
  // whoami
  whoami() {
    return { success: true, output: "player" };
  }

  // uname
  uname(args = []) {
    if (args.includes("-a")) {
      return { success: true, output: "Linux terminalquest 5.15.0 #1 SMP x86_64 GNU/Linux" };
    }
