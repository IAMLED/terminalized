...
export function executeCommand(parsed, vfs, level, onScore, currentLevelScore) {
  ...
  if (cmd === "help") {
    ...
    return {
      output: `Terminal Quest — Level ${level.id} Commands\n${"=".repeat(40)}\n` +
        cmds.map(c => `  ${c}`).join("\n") +
  ...
  if (result.success && LEARNING_COMMANDS.has(cmd)) {
    ...
    if (rawOutput.includes(level.flag)) {
      scoreIfAllowed(SCORING.findFlag);
      return {
        output: rawOutput + `\n\n\x1b[32m🚩 FLAG CAPTURED: ${level.flag}\x1b[0m\nLevel complete! Preparing next mission...`,
        isError: false, flagFound: true, correct: true,
      };
    }
  }
